    ...
    Just (FRFSRouteAPI route') -> 
      ...
      , onClick push $ const $ SelectQuote (FrfsQuote quote)
        ...
        , gravity LEFT
          ...
          , fastestRoutePillView state route'.shortName
        ]
      , linearLayout
        [ width MATCH_PARENT
        , height WRAP_CONTENT
...
fastestRoutePillView :: forall w. SD.SelectBusRouteScreenState -> String -> PrestoDOM (Effect Unit) w
fastestRoutePillView state routeCode = 
  linearLayout
    [ width WRAP_CONTENT
    , height WRAP_CONTENT
    , cornerRadius 20.0
